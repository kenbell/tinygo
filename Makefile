...
ifeq ($(OS),Windows_NT)
    ...
    CGO_LDFLAGS += $(abspath llvm-build/tools/clang/tools/libclang/CMakeFiles/libclang.dir/CXType.cpp.obj)
...
tinygo:
	...
	CGO_CPPFLAGS="$(CGO_CPPFLAGS)" CGO_CXXFLAGS="$(CGO_CXXFLAGS)" CGO_LDFLAGS="$(CGO_LDFLAGS)" $(GO) build -buildmode exe -x -o build/tinygo$(EXE) -tags byollvm -ldflags="-X main.gitSha1=`git rev-parse --short HEAD`" .
