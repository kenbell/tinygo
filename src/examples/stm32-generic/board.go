...
func (b *board) initClocks() {
	...
	oscCfg := clock.OscillatorConfig{
		Types:    clock.OscillatorTypeHSE,
		HSEState: clock.HSEStateBypass,
		HSIState: clock.HSIStateOff,
		PLL: clock.PLLConfig{
			Source: clock.PLLSourceHSE,
			State:  clock.PLLStateOn,
			M:      PLL_M,
			N:      PLL_N,
			P:      PLL_P,
			Q:      PLL_Q,
	...
	clock.HSE.Frequency = HSE_FREQ
	clock.SYSCLK.Frequency = SYSCLK_FREQ
	clock.HCLK.Frequency = HCLK_FREQ
	clock.PCLK1.Frequency = PCLK1_FREQ
	clock.PCLK2.Frequency = PCLK2_FREQ
}
